...
const registerUser = asyncHandler(async (req, res) => {
  ...
  try {
    if (coverLocalPath) {
      ...
      coverImage = null; // Handle case where cover image is not uploaded
    }
  } catch (error) {
  ...
  try {
    ...
    // Cleanup Cloudinary images if user creation fails
    if (avatar) {
      try {
    ...
    if (coverImage) {
      try {
        ...
        console.log("Error deleting cover image from Cloudinary", err);
      }
    }
...
const updateAccountDetails = asyncHandler(async (req, res) => {
  ...
  if (!fullname || !email) {
    throw new ApiError(400, "Fullname and eamil are required");
  }
